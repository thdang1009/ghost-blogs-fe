...
export class GoogleAuthService {
  private googleClientId =
    '67382640406-n2t19su323cdfrkhvi72dmcg4abd2pg5.apps.googleusercontent.com';
    ...
    if (response.credential) {
      ...
      this.verifyGoogleToken(response.credential).subscribe(
        result => {
          ...
          // Handle successful login - cookies are set automatically by backend
          if (result.status) {
            console.log('verifyGoogleToken result', result);
  ...
  verifyGoogleToken(idToken: string): Observable<GoogleLoginResponse> {
    ...
    return this.http.post<GoogleLoginResponse>(`${apiUrlGoogle}`, request).pipe(
      tap(resp => {
